...
const Room = () => {
  ...
  // ✅ Helper to get username for both guest and logged-in users
  const getUsername = () => {
    if (isLoggedIn) return localStorage.getItem("username") || "User";
  ...
  // Controls
  const handleToggleMic = () => { toggleMic(); setIsMicOn(prev => !prev); };
  const handleToggleCamera = () => { toggleCamera(); setIsCameraOn(prev => !prev); };
  const handleScreenShare = async () => {
    try { await startScreenShare(); setIsSharing(prev => !prev); }
  ...
  if (error) {
    return (
      <div className="flex items-center justify-center h-screen bg-gray-900 text-white">
        <div className="text-center">
          <p className="text-red-500 text-xl mb-4">Error: {error}</p>
          <button onClick={() => navigate("/")} className="px-6 py-2 bg-blue-600 rounded hover:bg-blue-500">
            Go Home
          </button>
        </div>
      </div>
  ...
  return (
    <div className="flex flex-col lg:flex-row h-screen w-full text-white bg-black">
      {/* VIDEO SECTION */}
      <div className="flex-1 relative flex flex-col lg:flex-row">
        {!remoteStream ? (
          ...
          <>
            <video ref={remoteVideoRef} autoPlay playsInline className="w-full lg:w-1/2 h-1/2 lg:h-full object-cover" />
            <video ref={localVideoRef} autoPlay playsInline muted className="w-full lg:w-1/2 h-1/2 lg:h-full object-cover border border-gray-500" />
          </>
        )}

        {/* CONTROLS */}
        <div className="absolute bottom-5 left-1/2 -translate-x-1/2 flex gap-3 lg:gap-6">
          <button onClick={handleToggleMic} className={`flex items-center justify-center w-12 h-12 rounded-full ${isMicOn ? "bg-gray-700 hover:bg-gray-600" : "bg-red-600 hover:bg-red-500"}`}>
            {isMicOn ? <FaMicrophone className="w-5 h-5" /> : <FaMicrophoneSlash className="w-5 h-5" />}
          </button>
          <button onClick={handleToggleCamera} className={`flex items-center justify-center w-12 h-12 rounded-full ${isCameraOn ? "bg-gray-700 hover:bg-gray-600" : "bg-red-600 hover:bg-red-500"}`}>
            {isCameraOn ? <IoVideocam className="w-5 h-5" /> : <IoVideocamOff className="w-5 h-5" />}
          </button>
          <button onClick={handleScreenShare} className={`flex items-center justify-center w-12 h-12 rounded-full ${isSharing ? "bg-green-600 hover:bg-green-500" : "bg-blue-600 hover:bg-blue-500"}`}>
            <MdScreenShare className="w-5 h-5" />
          </button>
          <button onClick={handleEndCall} className="flex items-center justify-center w-12 h-12 rounded-full bg-red-600 hover:bg-red-500">
            <MdCallEnd className="w-5 h-5" />
          </button>
          <button onClick={() => setIsChatOpen(prev => !prev)} className="flex items-center justify-center w-12 h-12 rounded-full bg-gray-700 hover:bg-gray-600 lg:hidden">
            <FaCommentDots className="w-5 h-5" />
          </button>
        </div>
      </div>
      ...
      {/* CHAT SECTION */}
      <div className={`fixed lg:relative top-0 right-0 h-full w-full lg:w-[440px] bg-gray-900 z-50 transform transition-transform duration-300 ${isChatOpen ? "translate-x-0" : "translate-x-full lg:translate-x-0"}`}>
        <div className="flex flex-col h-full">
          <div className="p-4 border-b border-gray-700 flex justify-between items-center">
            <div>
              <h3 className="text-xl font-bold">Chat</h3>
            ...
            <button className="lg:hidden text-xl hover:text-gray-300" onClick={() => setIsChatOpen(false)}>✖</button>
          </div>

          <div ref={chatContainerRef} className="flex-1 overflow-y-auto p-4 space-y-3 bg-gray-800">
            {messages.length === 0 ? (
              <div className="text-center text-gray-400 mt-8">
                <FaCommentDots className="w-12 h-12 mx-auto mb-2 opacity-50" />
                <p>No messages yet. Start the conversation!</p>
              ...
              messages.map((m, i) => (
                <div key={i} className={`flex ${m.sender === username ? "justify-end" : "justify-start"}`}>
                  <div className={`max-w-[80%] rounded-lg px-3 py-2 ${m.sender === username ? "bg-blue-600 text-white" : "bg-gray-700 text-white"}`}>
                    {m.sender !== username && <div className="text-xs font-semibold mb-1 text-blue-300">{m.sender}</div>}
                    <div className="break-words whitespace-pre-wrap">{m.msg}</div>
                  </div>
                </div>
              ))
          ...
          <div className="p-3 border-t border-gray-700 flex items-end bg-gray-900">
            <textarea
              ...
              rows={1}
            />
            <button onClick={handleSendMessage} disabled={!messageInput.trim()} className="px-4 py-2 bg-blue-600 rounded-r hover:bg-blue-500 disabled:bg-gray-600 disabled:cursor-not-allowed">
              Send
            </button>
          </div>
        </div>
